    ...
    public function createPod($name = 'voice_changer_A40')
    {
        ...
        foreach ($gpus as $gpu) {
            ...
            $vars = [
                ...
                'coquiAgreed' => "1",
                'dockerArgs' => "bash -c 'set -e && echo \"[1/3] Cloning/Updating Codebase...\" && (if [ ! -d \"/workspace/voice-changer\" ]; then cd /workspace && git clone --depth 1 -b diki https://github.com/dikidag541/voice_changer voice-changer; else cd /workspace/voice-changer && git pull origin diki; fi) && cd /workspace/voice-changer/ai-training-runpod && echo \"[2/3] Setting up Persistent Environment...\" && (if [ ! -d \"/workspace/venv\" ]; then echo \"🆕 Creating new venv (First time only)...\" && python3 -m venv --system-site-packages /workspace/venv && /workspace/venv/bin/pip install --upgrade pip && /workspace/venv/bin/pip install --no-cache-dir -r requirements.txt; else echo \"✅ Using existing venv...\"; fi) && echo \"[3/3] Starting AI Server...\" && /workspace/venv/bin/python3 -m uvicorn api.server:app --host 0.0.0.0 --port 8888'"
            ];
