    ...
    public function createPod($name = 'voice_changer_A40')
    {
        ...
        foreach ($gpus as $gpu) {
            ...
            $vars = [
                ...
                'coquiAgreed' => "1",
                'dockerArgs' => "bash -c 'set -e && echo \"[1/4] Installing system dependencies...\" && apt-get update && apt-get install -y ffmpeg git git-lfs espeak-ng build-essential g++ && echo \"[2/4] Cloning codebase...\" && (if [ ! -d \"/workspace/voice-changer\" ]; then cd /workspace && git clone --depth 1 -b diki https://github.com/dikidag541/voice_changer voice-changer; else cd /workspace/voice-changer && git pull origin diki; fi) && echo \"[3/4] Installing AI libraries (Optimized)...\" && cd /workspace/voice-changer/ai-training-runpod && pip install --upgrade pip && pip install --no-cache-dir -r requirements.txt && echo \"[4/4] Starting training server...\" && python3 -m uvicorn api.server:app --host 0.0.0.0 --port 8888'"
            ];
