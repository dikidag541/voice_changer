    ...
    public function createPod($name = 'voice_changer_A40')
    {
        ...
        foreach ($gpus as $gpu) {
            ...
            $vars = [
                ...
                'coquiAgreed' => "1",
                'dockerArgs' => "bash -c 'set -e && echo \"[1/3] Syncing Codebase...\" && (if [ ! -d \"/workspace/voice-changer\" ]; then cd /workspace && git clone --depth 1 -b diki https://github.com/dikidag541/voice_changer voice-changer; else cd /workspace/voice-changer && git pull origin diki; fi) && cd /workspace/voice-changer/ai-training-runpod && if [ ! -f \"/workspace/venv/bin/uvicorn\" ]; then echo \"🆕 Environment corrupted or missing. Re-installing...\" && rm -rf /workspace/venv && apt-get update && apt-get install -y ffmpeg espeak-ng build-essential g++ python3-venv && python3 -m venv --system-site-packages /workspace/venv && /workspace/venv/bin/pip install --upgrade pip && /workspace/venv/bin/pip install --no-cache-dir uvicorn fastapi TTS trainer requests fairseq praat-parselmouth pyworld faiss-cpu scipy librosa soundfile boto3 python-dotenv openai-whisper; fi && echo \"[3/3] Starting Server...\" && /workspace/venv/bin/python3 -m uvicorn api.server:app --host 0.0.0.0 --port 8888'"
            ];
